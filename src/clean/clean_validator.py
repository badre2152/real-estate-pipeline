...
Usage (called automatically inside run_clean):
    from src.clean.clean_validator import validate_pre_clean, validate_post_clean, CleanValidationError # noqa: E501
...
def _pre_min_rows(df: pd.DataFrame) -> None:
    ...
    logger.info(
        f"  ✅ staging row count: {len(df)} (≥ {HARD_MIN_STAGING_ROWS})")
...
def _pre_no_all_null_rows(df: pd.DataFrame) -> None:
    ...
    if all_null > 0:
        ...
        logger.info("  ✅ no fully-null rows in staging")
...
def _pre_lien_format(df: pd.DataFrame) -> None:
    ...
    if len(bad) > 0:
        ...
        logger.info("  ✅ all staging liens point to avito.ma")
...
def _post_required_columns(df: pd.DataFrame) -> None:
    ...
    if missing:
        raise CleanValidationError(
            ...
            "A transformation step may have failed silently."
        )
...
def _post_no_raw_surface_col(df: pd.DataFrame) -> None:
    ...
    logger.info("  ✅ raw 'surface' column correctly dropped")
...
def _post_prix_positive(df: pd.DataFrame) -> None:
    ...
    if len(bad) > 0:
        raise CleanValidationError(
            ...
            "The _apply_missing_value_strategy filter failed. "
            f"Values: {bad['prix'].tolist()}"
        )
    logger.info("  ✅ all prix values are positive")
...
def _post_no_duplicate_liens(df: pd.DataFrame) -> None:
    ...
    logger.info("  ✅ no duplicate liens in cleaned output")
...
def _post_no_artefact_villes(df: pd.DataFrame) -> None:
    ...
    if len(remaining) > 0:
        raise CleanValidationError(
            ...
            "(e.g. 'COURS ET FORMATIONS'). "
            "_apply_missing_value_strategy must filter these."
        )
    logger.info("  ✅ no artefact villes survived cleaning")
...
def _post_prix_type_values(df: pd.DataFrame) -> None:
    ...
    if len(invalid) > 0:
        ...
        logger.info("  ✅ all prix_type values valid")
...
def _post_categorie_prix_values(df: pd.DataFrame) -> None:
    ...
    if "categorie_prix" not in df.columns:
        return
    invalid = df[df["categorie_prix"].notna(
    ) & ~df["categorie_prix"].isin(VALID_CATEGORIES)]
    if len(invalid) > 0:
        logger.warning(
        ...
        logger.info("  ✅ all categorie_prix values valid")
...
def _post_region_label_values(df: pd.DataFrame) -> None:
    ...
    if "region_label" not in df.columns:
        return
    invalid = df[df["region_label"].notna(
    ) & ~df["region_label"].isin(VALID_REGIONS)]
    if len(invalid) > 0:
        logger.warning(
        ...
        logger.info("  ✅ all region_label values valid")
...
def _post_prix_par_m2_consistency(df: pd.DataFrame) -> None:
    ...
    if len(check) == 0:
        logger.info("  ✅ prix_par_m2 consistency: no rows to check")
        return
    ...
    if len(bad) > 0:
        logger.warning(
            ...
            "prix / surface_m2 (tolerance = 1 DH/m²). "
            f"Max deviation: {delta.max():.2f} DH/m²"
        )
    else:
        logger.info(
            "  ✅ prix_par_m2 is consistent with prix / surface_m2 on all rows")
...
def _post_age_bien_valid(df: pd.DataFrame) -> None:
    ...
    if len(bad_negative) > 0:
        ...
        logger.info("  ✅ age_bien values are all in valid range")
...
def _post_surface_positive(df: pd.DataFrame) -> None:
    ...
    if len(bad) > 0:
        ...
        logger.info("  ✅ all surface_m2 values are positive")
...
def _post_scraped_at_valid(df: pd.DataFrame) -> None:
    ...
    if len(future) > 0:
        logger.warning(
            f"  ⚠️  {len(future)} row(s) have future scraped_at timestamps")
    elif len(null_ts) > 0:
        logger.warning(
            f"  ⚠️  {len(null_ts)} row(s) have unparseable scraped_at")
    else:
        logger.info("  ✅ all scraped_at timestamps are valid and in the past")
...
def validate_pre_clean(df_staging: pd.DataFrame) -> None:
    ...
    for name, fn in hard_rules:
        try:
            ...
            raise CleanValidationError(
                f"Unexpected error in pre-clean rule '{name}': {e}") from e
