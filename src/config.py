...
MAX_PAGES       = int(os.getenv("MAX_PAGES",  "25"))   # pages per run (~20 listings/page)
DELAY_MIN       = float(os.getenv("DELAY_MIN", "2.5")) # min seconds between detail requests
DELAY_MAX       = float(os.getenv("DELAY_MAX", "5.0")) # max seconds between detail requests
SCRAPER_TIMEOUT = int(os.getenv("SCRAPER_TIMEOUT", "5"))  # seconds to sleep on empty page
...
HARD_MIN_PRIX_FILL_STAGING = float(os.getenv("HARD_MIN_PRIX_FILL_STAGING", "0.50"))
SOFT_PRIX_M2_FILL        = float(os.getenv("SOFT_PRIX_M2_FILL",      "0.50"))

# ── Price & surface ranges ─────────────────────────────────────────────────────
MIN_PRIX     = float(os.getenv("MIN_PRIX",    "100"))      # DH — below this = artefact
MAX_PRIX     = float(os.getenv("MAX_PRIX",    "500000"))   # DH — above this = outlier
MIN_SURFACE  = float(os.getenv("MIN_SURFACE", "5"))        # m² — below this = artefact
...
PIPELINE_RETRY_DELAY = int(os.getenv("PIPELINE_RETRY_DELAY", "10"))  # base seconds (exponential backoff)
